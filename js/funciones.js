...
class Empleado{
	...
	investigar_fisico(est_peso,est_altura){
		...
		let est_fisico = ((((this.peso)/30)/8)*2)*(this.altura);
		if(est_fisico < 0){
			this.estado='peso inferior al normal'+ est_fisico;
		} else{
			if((est_fisico > 0)&&(est_fisico < 24.9)){
				this.estado='normal'+ est_fisico;
			} else{
				if((est_fisico > 1)&&(est_fisico < 29.9)){
					this.estado='peso superior al normal'+ est_fisico;
				} else{
					this.estado='El valor de sus horas es    '+ est_fisico;
				}
			}
		}
	}
...
function mostrar_cargo(){
	...
	document.getElementById("cargo_que_ocupa").innerHTML = "Usted se encuentra en la  :" + puesto_actu;
}
...
function mostrar_estado(){
	...
	document.getElementById("estado_fisico").innerHTML = "" + esta_actu;
}
